...
module.exports = {
    get: (req, res)=> {
      return new Promise((resolve, reject)=> {
        ...
        const sql = `SELECT * FROM booking ${title ? `WHERE title LIKE '%${title}%'`: title && director ? `WHERE title LIKE '%${title}%' AND director LIKE '${director}%'`:''} ORDER BY release_date DESC`
    ...
    update: (req, res) => {
      return new Promise((resolve, reject)=> {
        ...
        db.query(`SELECT * FROM booking where id=${id}`,(err, results)=> {
          ...
          db.query(`UPDATE booking SET title='${title}', cover='${cover}', release_date='${release_date}', director='${director}', description='${description}', casts='${casts}', categories='${categories}'`,(err, results)=> {
            if(err) {
              console.log(err)
              reject({message: "ada error"})
            }
